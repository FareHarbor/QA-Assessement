...
class bookings {
    ...
    addPeopleForBigApple(attendee = 0) {
        cy.waitForAPIRequest();
        cy.get(ATTENDEE.BIG_APPLE_ATTENDEE).within(() => {
            cy.get('option').each(($e) => {
                if ($e.text() == attendee) {
                    cy.wrap($e).parent().select(attendee.toString())
                }
            })
        })
    }

    selectCateringOption(optionToSelect) {
        cy.waitForAPIRequest()
        cy.get(ATTENDEE.CATERING_OPTION).within(() => {
            cy.get('select').each(($e) => {
                cy.wrap($e).select(optionToSelect, { force: true })
            })
        })
    }

    //Improvement: This method can Replace fillContactInformation method above
    addContactDetails() {
